...
- 连接串统一来自 ``app.core.config.settings``（可用 ``-x dsn=...`` 覆盖，便于测试）
- 先 ``import app.db.models`` 保证 ``Base.metadata`` 含全部 30 张表
"""
...
# 保证 `alembic` 在 backend/ 目录下执行时能 import app 包
...
import app.db.models  # noqa: E402,F401  注册全部模型
