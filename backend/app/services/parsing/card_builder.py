...
- **结论性条目必须带 evidence**：模型的 ``evidence_quote`` 只是"候选引用"，由
  :mod:`app.services.parsing.locator` 回原文核对；核对不上就 ``evidence_span=null``。
...
from app.services.fulltext.text_cache import default_text_cache
...
async def call_card_llm(
    ...
    *,
    ...
    """调用 ``app.llm.chat``（``stage='parse'``/``purpose='card_build'``）并按 ``LLM_JSON_RETRY`` 重试。

    返回 ``(卡片, 调用审计, {'linked_id': ...})``；``linked_id`` 为最后一次成功调用写入
    ``llm_call_logs`` 的行 id，用于 ``paper_cards.llm_call_log_id`` 关联。
    """
    from app.llm import chat
    from app.llm.replay import json_retry_limit as _json_retry_limit
...
def publish_card_progress(paper_id: int, percent: int, message: str) -> None:
    ...
    try:
        from app.services.pipeline.sse import publish
