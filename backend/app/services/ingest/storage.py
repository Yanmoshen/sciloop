...
默认 ``<backend_root>/.cache/uploads``（容器内即 ``/app/backend/.cache/uploads``，
...
def default_upload_dir() -> Path:
    """默认上传目录：``<backend_root>/.cache/uploads``。

    ``app/services/ingest/storage.py`` → parents[3] == ``<backend_root>``，
    容器内即 ``/app/backend``（与 compose 的挂载点一致），本地跑则是 ``backend/``。
    """
    override = os.environ.get("IMPORT_UPLOAD_DIR")
    if override:
        return Path(override)
    return Path(__file__).resolve().parents[3] / ".cache" / "uploads"
