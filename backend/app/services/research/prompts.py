...
from app.services.research.rules import MIN_EVIDENCE_COUNT, rule_catalog

logger = logging.getLogger("sciloop.research.prompts")

__all__ = ["NODE_GUIDES", "SYSTEM_PROMPT", "build_messages"]

#: 节点 → 长文档文件名（人类可读版在 research-workflow-prompts/）
...
def _candidate_dirs() -> list[Path]:
    ...
    for base in list(here.parents)[:6]:
        dirs.append(base / "research-workflow-prompts")
    return dirs
...
def _format_contract(node: str) -> str:
    from app.services.research.contracts import NODE_OUTPUT_SCHEMAS
