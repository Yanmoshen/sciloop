...
- 哈希取的是**归一化文本**（大小写、行内空白、断词连字符已由
  :func:`app.services.fulltext.records.normalize_block_text` 与 pymupdf 解析器处理），
  排版微调不会打散 id。
...
from app.services.fulltext import NoTextLayerError, parse_pdf

#: ``normalize_block_text`` 未在 ``app.services.fulltext`` 顶层导出，按子模块引用
from app.services.fulltext.records import normalize_block_text
