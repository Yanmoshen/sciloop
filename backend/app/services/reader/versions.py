...
----------------------------------------
``.cache/artifacts/<task_id>/manifest.json``（容器内 ``/app/backend/.cache/artifacts``）
...
from app.services.reader import parsing, storage
...
def artifacts_root() -> Path:
    """翻译产物根：``<backend_root>/.cache/artifacts``。

    与 ``app/services/translate/artifacts.py`` 使用**同一环境变量**
