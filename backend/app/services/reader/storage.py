...
目录布局（持久化，禁止放临时目录）::
    ...
    .cache/reader-library/                       # 容器内 /app/backend/.cache/reader-library
...
def default_library_dir() -> Path:
    """默认库根：``<backend_root>/.cache/reader-library``。

    ``app/services/reader/storage.py`` → ``parents[3]`` == ``<backend_root>``，
    容器内即 ``/app/backend``（与 docker-compose 的 bind mount 完全对应：
    宿主 ``./.data/reader-library``）。可用 ``READER_LIBRARY_DIR`` 覆盖（测试/离线验证）。
    """
    override = os.environ.get("READER_LIBRARY_DIR")
    if override:
        return Path(override)
    return Path(__file__).resolve().parents[3] / ".cache" / "reader-library"
