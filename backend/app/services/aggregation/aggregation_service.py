...
1. ``comparison_matrix`` —— :mod:`app.services.aggregation.matrix`（真实卡片字段）
2. ``method_evolution``  —— :mod:`app.services.aggregation.evolution`（真实卡片 + span 重叠）
3. ``gaps``              —— :mod:`app.services.aggregation.gap_finder`（真实 span / card_field）
...
from app.db.models import Aggregation, Gap
from app.services.aggregation.cards import existing_paper_ids, load_cards
from app.services.aggregation.evolution import build_evolution_payload
from app.services.aggregation.gap_finder import build_gaps_payload
from app.services.aggregation.matrix import MAX_PAPERS, MIN_PAPERS, build_matrix_payload
