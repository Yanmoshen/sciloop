...
------------------------------------------------
``llm``
    走 WP02 :func:`app.llm.adapter.chat`（结构化输出 + 每次调用写 ``llm_call_logs``）。
...
最后统一跑 :func:`app.services.ideation.evidence_binder.enforce_evidence_or_drop`：
...
from app.services.aggregation.aggregation_service import load_gaps
from app.services.cost import check_cost
...
async def _llm_ideas(
    ...
    *,
    ...
    from app.llm.adapter import chat  # 局部导入：避免无 LLM 场景下引入导入期依赖
...
async def generate_ideas(
    ...
    *,
    ...
    from app.services.ideation.idea_service import load_idea  # 局部导入避免循环
