...
1. :func:`app.executor.execute_template` —— 受限执行器跑模板（限额 / 白名单 / 超时）
2. :mod:`app.services.experiment.metrics` —— 真实指标落 ``experiment_metrics``
3. 回填 ``stage_outputs.output_json``（有 ``pipeline_run_id`` 时）
4. :mod:`app.services.experiment.passport` —— 生成不可变 Passport
...
from app.executor import RunOutcome, execute_template
from app.services.experiment import metrics as M
from app.services.experiment import passport as passport_mod
...
async def list_templates(*, implemented_only: bool = False) -> list[dict[str, Any]]:
    ...
    from app.services.experiment import registry
...
async def list_experiments(*, session: Any = None, limit: int = 100) -> list[dict[str, Any]]:
    ...
    from app.db.models.pipeline import Experiment
...
async def get_run(run_id: int, *, session: Any = None) -> dict[str, Any]:
    ...
    from app.db.models.pipeline import ExperimentRun
