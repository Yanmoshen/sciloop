...
def effective_replay_enabled() -> bool:
    """**服务端实际生效**的 replay 开关（与 ``app.llm.replay`` 同口径）。"""
...
def get_demo_state() -> DemoRuntimeState:
    """进程内单例（与 ``app.llm.replay.is_replay_enabled`` 共用同一份语义）。"""
    return _state
...
def _effect_block() -> dict[str, Any]:
    ...
    return {
        "replay": {
            ...
            "consumed_by": "app.llm.replay.is_replay_enabled（先读环境变量，运行期生效）",
        ...
        "access_mode": {
            ...
            "consumed_by": "app.core.security.require_owner + app.services.demo.access.evaluate_request",
