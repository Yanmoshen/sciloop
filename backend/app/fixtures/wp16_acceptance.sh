...
#
# 用法（仓库根目录执行）：
#   bash backend/app/fixtures/wp16_acceptance.sh                 # 全量
#   bash backend/app/fixtures/wp16_acceptance.sh A2 A3 A7        # 只跑指定项
#
...
if want A2; then
  ...
  # a) 前端产物
  if docker compose exec -T frontend sh -c 'grep -rIl "$1" /usr/share/nginx/html 2>/dev/null' _ "${OWNER_TOKEN:-__unset__}" >"$OUT_DIR/A2_frontend_hits.txt" 2>&1; then
    n=$(grep -c . "$OUT_DIR/A2_frontend_hits.txt" || true)
    if [ "${n:-0}" -eq 0 ]; then ok "A2 前端产物（frontend/dist, nginx html）未命中令牌"
    else bad "A2 前端产物命中令牌 $n 处，见 $OUT_DIR/A2_frontend_hits.txt"; fi
  else
    ok "A2 前端产物未命中令牌（grep 无匹配）"
  fi
  # b) 前端源码（防止令牌被写进仓库代码/构建参数）
  if [ -n "${OWNER_TOKEN:-}" ]; then
    if grep -rIl --exclude-dir=node_modules --exclude-dir=dist -e "$OWNER_TOKEN" frontend backend/app 2>/dev/null | tee "$OUT_DIR/A2_source_hits.txt" | grep -q .; then
      bad "A2 源码命中令牌，见 $OUT_DIR/A2_source_hits.txt"
    else
      ok "A2 frontend/src 与 backend/app 源码未命中令牌"
    fi
  else
...
if want A4; then
  ...
  docker compose exec -T -e LLM_REPLAY=1 -e LLM_DEFAULT_API_KEY= -e LLM_FALLBACK_API_KEY= backend \
    python -m app.services.demo.replay --replay-check --project-id "${WP16_DEMO_PROJECT_ID:-41}" --out /tmp/A4.json \
    >"$rep" 2>&1
  status=$?
...
if want A5 || want A6; then
  ...
  docker compose exec -T backend python -m app.tasks.jobs.seed_demo --verify >"$rep" 2>&1 || true
