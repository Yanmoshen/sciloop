...
- 切换后立刻广播 ``demo_mode {snapshot, replay}``，使前端 DemoBadge 立即反映
  （``app.services.pipeline.sse`` 已订阅该事件）
"""
...
from app.services.demo.access import get_replay_limiter, public_demo_summary
from app.services.demo.state import get_demo_state, state_public_view
...
def _demo_project_inventory() -> dict[str, Any]:
    ...
    try:
        from app.services.demo.snapshot import demo_project_inventory
...
def build_status(request: Request | None = None) -> dict[str, Any]:
    ...
    if request is not None:
        try:
            from app.services.demo.access import _request_client_key
...
async def demo_mode(payload: DemoModeRequest, _: OwnerDep) -> dict[str, Any]:
    ...
    from app.llm.events import emit_demo_mode
...
async def demo_seed(payload: SeedRequest, _: OwnerDep) -> dict[str, Any]:
    ...
    """
    from app.tasks.jobs.seed_demo import run_seed_sync
...
async def demo_projects(
    ...
    try:
        from app.services.demo.snapshot import list_projects
