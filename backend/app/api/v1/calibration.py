...
本模块**同时负责注册 ``plan_review`` 环节**：``app.main.ROUTER_REGISTRY`` 已预留
``app.api.v1.calibration``（模块不存在则跳过），因此这里的导入副作用就是环节注入点，
``main.py`` 无需任何改动。
...
from app.services.review import calibration as calibration_mod
...
def _register_plan_review() -> str:
    ...
    try:
        from app.services.pipeline.stages.plan_review import register
