...
- ``python -m app.tasks.jobs.fetch_papers --limit 100``（手动跑一轮并打印报告）
"""
...
from app.services.paper_source.cache import cache_stats, close_all_clients
from app.services.paper_source.openalex_client import OpenAlexUnavailable
from app.services.paper_source.s2_client import S2Unavailable
...
def history_path() -> Path:
    ...
    base = Path(raw) if raw else Path(__file__).resolve().parents[3] / ".cache" / "tasks"
