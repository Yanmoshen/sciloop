...
"""``app.tasks.jobs`` —— 批量任务包（显式注册表，避免「同名包空文件」造成的 import 歧义）。
...
三个调度任务（由 :mod:`app.tasks.scheduler` 注册，默认关闭）
----------------------------------------------------------

=================  ====================================  =========================
任务名             模块                                  同步入口
=================  ====================================  =========================
``fetch_papers``   ``app.tasks.jobs.fetch_papers``       ``run_fetch_sync``
``score_papers``   ``app.tasks.jobs.score_papers``       ``run_score_job``
``parse_fulltext`` ``app.tasks.jobs.parse_fulltext``      ``run_parse_fulltext_sync``
=================  ====================================  =========================
...
因此统一用模块方式执行（**不要**用 ``python app/tasks/jobs/x.py``，那样会丢包上下文）::
    ...
    docker compose exec backend python -m app.tasks.jobs.fetch_papers --limit 50
    docker compose exec backend python -m app.tasks.jobs.score_papers 200 --force
    docker compose exec backend python -m app.tasks.jobs.parse_fulltext --limit 50
    docker compose exec backend python -m app.tasks.jobs.seed_demo --help

本包的 ``__init__`` **不** eager import 任何任务模块：任务模块会拉起 httpx / pymupdf /
LLM 适配层等重依赖，任何一个缺依赖都不应该让 ``import app.tasks.jobs`` 失败
（调度器按模块粒度做惰性 import 并只告警，见 ``app.tasks.scheduler``）。
需要模块对象时用 :func:`load_job`，或直接 ``from app.tasks.jobs import fetch_papers``
...
#: 任务名 → 模块路径（任务名与 :mod:`app.tasks.scheduler` 的 ``job_id`` 同名同义）
JOB_MODULES: dict[str, str] = {
    "fetch_papers": "app.tasks.jobs.fetch_papers",
    "score_papers": "app.tasks.jobs.score_papers",
    "parse_fulltext": "app.tasks.jobs.parse_fulltext",
}

#: 非调度型运维模块（手工执行，不进 APScheduler）
AUX_MODULES: dict[str, str] = {
    "seed_demo": "app.tasks.jobs.seed_demo",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    约定：``python -m app.tasks.jobs.<name>`` 直接跑某个任务；
    ``run_job(name, argv)`` 供脚本/Python 侧调用同一入口。
    （若需要 ``python -m app.tasks.jobs`` 这种「包级索引」入口，需在同目录新增
    ...
    lines = [
        ...
        "",
        "调度任务（SCHEDULER_ENABLED=1 时由 app.tasks.scheduler 注册）：",
    ]
    ...
    lines.extend(["", "索引调用：python -m app.tasks.jobs <job_name> [args...]"])
    print("\n".join(lines))
    return 0


def __getattr__(name: str) -> Any:
    """让 ``from app.tasks.jobs import fetch_papers`` 惰性可用（PEP 562）。"""
