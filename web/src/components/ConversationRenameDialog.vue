 ...
 *
 * 对话重命名弹窗（**总览壳层专用样式**，用 `.sl-home` 的 --h-* 令牌）。
 *
 * 与 `ProjectRenameDialog.vue` 同口径（同样的居中遮罩、同样的字段与按钮），差别只有：
 * 写的是 `PATCH /conversations/{id}` 的 `title`。两者视觉一致，改样式时需一起改。
 * 403 时如实说明需要 OWNER_TOKEN，不做本地假装成功。
 */
...
watch(
  ...
  (open) => {
    if (!open) return
    title.value = props.conversationTitle
    notice.value = ''
    busy.value = false
  },
)
...
<template>
  <Teleport to="body">
    <div v-if="modelValue" class="overlay sl-home" @click.self="close">
      <div class="dialog" role="dialog" aria-modal="true" aria-label="重命名对话">
        <h2 class="dialog__title">重命名对话</h2>
        <label class="field">
          <span>标题</span>
          <input v-model="title" type="text" maxlength="200" @keyup.enter="submit" />
        </label>
...
<style scoped>
/* 居中弹框：四周半透明遮罩 + 背景模糊（与项目重命名 / 项目详情弹框同一口径） */
.overlay {
  position: fixed;
  inset: 0;
  z-index: 2200;
