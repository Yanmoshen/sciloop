...
from db.models.paper import Paper, PaperDocument, PaperFeedSnapshot
from services.paper_source import influence_score as influence
from services.paper_source import ranking
...
def _sync_database_url() -> str | None:
    ...
    url: str | None = None
    try:
        from core.config import settings  # WP01 配置中心
...
def get_session_factory() -> sessionmaker | None:
    ...
    与批量任务 ``tasks.jobs.score_papers`` 共用，保证两者使用同一份驱动探测与连接池。
    """
