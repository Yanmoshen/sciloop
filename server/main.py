...
from tasks import scheduler as job_scheduler
...
ROUTER_REGISTRY: tuple[tuple[str, str, bool], ...] = (
    ...
    ("api.v1.translate", "/api/v1", False),
    ("api.v1.reader", "/api/v1", False),
    ("api.v1.exports", "/api/v1", False),
    ...
    ("api.v1.parses", "/api/v1", False),
    # 4) 论文检索/详情（WP03）：/papers/search /papers/fetch /papers/{id}
    ("api.v1.papers", "/api/v1", False),
    # 5) 全文解析记录（WP05）：/papers/{id}/documents /spans /parse
    ("api.v1.documents", "/api/v1", False),
    # 6) 构思域（WP08）：聚合 / 空白清单 / idea / 可行性 / 任务书
    ("api.v1.aggregations", "/api/v1", False),
    ("api.v1.ideas", "/api/v1", False),
    ("api.v1.feasibilities", "/api/v1", False),
    ("api.v1.taskbooks", "/api/v1", False),
    # 7) 流水线 / 决策 / SSE（WP09 / WP10）
    ("api.v1.pipelines", "/api/v1", False),
    ("api.v1.runs", "/api/v1", False),
    ("api.v1.stream", "/api/v1", False),
    ("api.v1.decisions", "/api/v1", False),
    # 8) 实验 / Passport / 盲评校准（WP11 / WP12）
    ("api.v1.experiments", "/api/v1", False),
    ("api.v1.passports", "/api/v1", False),
    ("api.v1.calibration", "/api/v1", False),
    # 9) 证据链与草稿（WP13 / WP14）
    ("api.v1.evidence", "/api/v1", False),
    ("api.v1.claims", "/api/v1", False),
    ("api.v1.drafts", "/api/v1", False),
    # 10) 项目容器（WP09 的 projects 端点；单列避免与 pipelines 前缀混淆）
    ("api.v1.projects", "/api/v1", False),
    # 11) 演示与访问面（WP16）
    ("api.v1.demo", "/api/v1", False),
    ("api.v1.owner", "/api/v1", False),
...
# 供 ``python -m main`` 本地起服（生产走 uvicorn 命令）
if __name__ == "__main__":  # pragma: no cover
    import uvicorn

    _start = time.time()
    uvicorn.run("main:app", host="0.0.0.0", port=8000, reload=False)  # noqa: S104
