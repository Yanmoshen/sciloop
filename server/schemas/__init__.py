...
"""``schemas`` —— SciLoop 对外契约与文档可引用层（Pydantic v2）。
...
- **不做业务校验**：除少量契约红线（如 evidence 必须带锚点、idea 必须带证据）外，
  分项归一、权重计算、闸门判定等一律留在各自 ``services.*``，本包不重复实现。

用法::

    from schemas import Paginated, Paper, ErrorBody
...
from schemas.routing import ModelConfigBrief, Routing, RoutingUpdateRequest
