...
回放读取端由 WP02 交付（``llm.replay`` + ``LLM_REPLAY=1``）。本模块负责**录制端**
与**覆盖率体检**，两者都必须与 ``llm.replay`` 的口径**逐字段一致**：

- ``fixture_key`` 一律取 ``LLMResult.prompt_hash`` —— 该值由 ``adapter.chat`` 调用
  :func:`llm.replay.compute_prompt_hash` 算出，**不要自己复算**：
  自己拼参数（尤其是 ``temperature`` / ``max_tokens`` 用「解析后的默认值」而非
  调用方传入的原始值）会让哈希永远对不上，回放 100% 未命中。
- ``payload`` 一律用 :func:`llm.replay.build_fixture_payload` 构造
...
1. **录制**（需真实 LLM 凭据）::
       ...
       python -m services.demo.replay --record --project-id 1

   它会把 ``llm.adapter.chat`` 临时包一层：强制 ``allow_replay=False``（走实时），
   每次成功调用后把响应写进 ``demo_fixtures``；跑完打印每个 ``prompt_hash`` 的落库结果。
   六环节的录制就是「用这个包装跑一次完整流水线」。

2. **体检**（无需凭据）::

       python -m services.demo.replay --inventory      # 库存与按环节分布
       python -m services.demo.replay --verify         # 逐条校验 payload 结构
       python -m services.demo.replay --replay-check --project-id 1
                                                          # 回放模式跑一次，列出**缺失键**
...
---------------------------------------------
``app/services/pipeline/stages/base.py`` 在函数体内 ``from llm.adapter import chat``，
因此运行期替换 ``llm.adapter.chat`` 能覆盖全部环节调用，且**不修改任何他包文件**。
包装只在录制进程内生效，录制结束即还原。
...
from llm.replay import PROMPT_HASH_VERSION, build_fixture_payload, payload_to_content
from llm.store import FIXTURE_TYPE_LLM_RESPONSE, get_store
...
def recording_adapter(recorder: Recorder) -> Iterator[Recorder]:
    """在上下文中把 ``llm.adapter.chat`` 包一层：实时调用 + 落 fixture。

    包装**强制** ``allow_replay=False``：录制时若命中回放会把回放结果再录一遍，
    形成「回放套回放」的自证循环，破坏 fixture 的真实来源。
    """
    from llm import adapter as adapter_module
    ...
    logger.info("录制模式已开启：llm.adapter.chat 已包装")
    try:
        yield recorder
    finally:
...
async def lookup(prompt_hash: str, *, store: Any | None = None) -> dict[str, Any]:
    ...
    from llm.replay import load_replay
...
async def collect_replay_misses(awaitable: Awaitable[Any]) -> tuple[Any, list[dict[str, Any]]]:
    ...
    from llm.events import default_bus
...
def replay_check_sync(*, project_id: int) -> dict[str, Any]:
    ...
    """
    from services.demo.state import get_demo_state
...
async def _replay_check(project_id: int) -> dict[str, Any]:
    try:
        from services.pipeline.engine import engine
    ...
    return {
        ...
        "next_step": (
            "缺失键需在录制模式下补录：python -m services.demo.replay "
