...
中间件是**可选安装**的：它需要 ``main`` 里加一行 ``install_access_guard(app)``
（``main.py`` 归 WP01，本包不修改）。未安装时不影响任何现有行为——当前所有写路由
都已带 ``require_owner``，安装只是把「忘挂依赖」这一类回归堵死。

``OWNER_TOKEN`` 只在 :mod:`core.security` 里与请求头比较，本模块**从不读取、
...
from core.security import OWNER_HEADER, owner_token_matches
...
def evaluate_request(
    *,
    ...
    :param owner_ok: 调用方用 :func:`core.security.owner_token_matches` 算好的结果
...
class AccessModeGuardMiddleware:
    ...
    async def __call__(self, scope: MutableMapping[str, Any], receive: Any, send: Any) -> None:
        ...
        from services.demo.state import resolve_access_mode
...
def install_access_guard(app: Any, *, enforce_in_owner_mode: bool = False) -> bool:
    ...
    供 ``main.restore_registry`` 之后加一行使用；**未安装也不影响现状**
...
def _iter_api_routes(app: Any) -> Iterable[tuple[str, Any, str | None]]:
    ...
    逐个 import ``main.ROUTER_REGISTRY`` 里的模块，取 ``module.router.routes``
    （平铺的 ``APIRoute``，带 ``dependant``），再拼上 ``/api/v1`` + router 前缀。
    这样审计结果与真实挂载一一对应，且不依赖 FastAPI 内部实现细节。
    """
    try:
        from main import ROUTER_REGISTRY
...
def audit_routes(app: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "本审计是静态分析（依赖树，按 main.ROUTER_REGISTRY 还原挂载路径），"
