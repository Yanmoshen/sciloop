...
容器内根目录为 ``/app/server/.cache/artifacts``（docker-compose 已把宿主
...
def default_artifact_root() -> Path:
    """默认产物根目录：``<server_root>/.cache/artifacts``。

    ``services/translate/artifacts.py`` → ``parents[2]`` == ``<server_root>``，
    容器内即 ``/app/server``（与 compose 的挂载点一致）。
    """
