...
2. **出网闸门**：``prepare_targets`` 解析模型 → **每个模板的每次 LLM 调用都先过
   :mod:`executor.egress_proxy` 白名单**（非白名单/内网直接失败）
...
from executor.egress_proxy import EgressPolicy, get_policy, host_of
from executor.errors import ModelUnavailableError, ReplaySourceMissError
...
class LocalStubTarget:
    """本地桩「模型」：结构与 :class:`llm.types.ResolvedModel` 对齐但**不是模型**。"""
...
async def _log_replay_call(
    ...
    *,
    ...
    try:
        from llm.store import get_store
