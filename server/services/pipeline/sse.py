...
2. 订阅 :data:`llm.events.default_bus`，把 WP02 的 ``llm_fallback`` / ``demo_mode`` /
...
def install_llm_forwarder() -> bool:
    """订阅 ``llm.events.default_bus`` 并把事件转发为 SSE（幂等）。"""
    ...
    try:
        ...
        logger.warning("无法导入 llm.events.default_bus，LLM 事件转发未启用")
        return False
    default_bus.subscribe(_forward_llm_event)
    _FORWARDER_INSTALLED = True
    logger.info("已订阅 llm.events.default_bus 并转发为 SSE（llm_fallback/demo_mode/llm_error/replay_miss）")
    return True
