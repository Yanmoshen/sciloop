...
原 ``llm.store.aggregate_costs`` 只按 ``stage`` 聚合、只区分 ``is_replay``，
无法按 ``provider/model`` 剔除桩调用（表结构无 ``is_stub`` 列，见 WP17 findings）。
本模块**只读**地按 ``(stage, provider, model, is_replay)`` 分桶，把 provider/model
原样带出，交给 :mod:`services.cost.classification` 判定口径。

- 生产路径：SQL（复用 :func:`llm.db.get_async_engine` 的连接池）
...
async def audit_cost_rows(
    ...
    *,
    ...
    """
    from services.cost.classification import classification_rules, classify_call
