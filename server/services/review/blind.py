...
1. **隔离**：``generator_model_ref != reviewer_model_ref`` 必须成立。校验**复用**
   WP02 的 :func:`llm.router.resolve_pair`，本模块不重写隔离逻辑；无法满足时抛
...
from llm.errors import IsolationViolation
from services.pipeline.stages.base import StageError
...
class IsolationStageError(StageError, IsolationViolation):
    ...
    - 契约与验收要求「报 ``IsolationViolation``」，因此保留
      :class:`~llm.errors.IsolationViolation` 的 ``isinstance`` 语义与
...
async def resolve_isolated_pair(project_id: int | None = None) -> tuple[Any, Any]:
    ...
    直接复用 WP02 的 ``llm.router.resolve_pair('plan', 'plan_review', project_id)``：
    ...
    """
    from llm.errors import ModelRoutingError
    from llm.router import resolve_pair
