...
对外契约（供 ``pipeline/stages/writing.py`` 与 ``api/v1/drafts.py`` 使用）::
    ...
    from services.writing import drafter, integrity_checker
...
- :mod:`services.writing.drafter` —— 证据池 / 大纲 / 分节撰写 / 引用编号与合规词表
- :mod:`services.writing.integrity_checker` —— Claim 拆分（优先调用 WP13）与三态校验落库
