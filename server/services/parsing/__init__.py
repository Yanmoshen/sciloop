...
对外契约（供 WP07 / WP08 / WP13 使用）::
    ...
    from services.parsing import build_card, get_card, list_card_versions
...
``evidence_span`` 由 :mod:`services.parsing.locator` 回原文核对后填入；
核对不上时``evidence_span=null`` 并计入 ``unlocated_count``（禁止编造引用）。
"""

from __future__ import annotations

from services.parsing.card_builder import (
