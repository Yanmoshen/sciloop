...
3. 未命中才取数：
   ...
   - ``arxiv``：走 :mod:`services.paper_source.arxiv_client`
     （``SOURCE_NAME`` / ``ARXIV_QPS`` / ``parse_feed``）＋该源共享的
     :func:`services.paper_source.cache.get_source_http`（同一份限流与缓存）；
...
from services.ingest.jobs import ITEM_CREATED, ITEM_FAILED, ITEM_REUSED
from services.paper_source import arxiv_client, identity, openalex_client, s2_client
