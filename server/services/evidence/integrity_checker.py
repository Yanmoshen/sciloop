...
async def _run(
    *,
    ...
    if persist and draft_found and draft_id is not None:
        from services.evidence.claim_splitter import persist_claims
...
async def verify_claims(
    ...
    *,
    ...
    :param conflict_detector: 冲突判定器（可接 WP02 的 ``llm.chat_json``）；
...
def make_llm_conflict_detector(
    *, model_ref: str | None = None, project_id: int | None = None
    ...
    """构造一个基于 WP02 ``llm.chat_json`` 的冲突判定器（需显式注入才生效）。
