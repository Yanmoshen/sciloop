...
stage: ideation
source: K-Dense-AI/scientific-agent-skills/skills/scientific-brainstorming
steps:
- id: evaluate_matrix
  title: evaluate matrix
  script: scripts/evaluate_matrix.py
  args:
  - '{{in}}'
  - --config
  - --weight-delta
  ...
  required: false
- id: session_scaffold
  title: session scaffold
  script: scripts/session_scaffold.py
  args:
  - '{{in}}'
  - --session-id
  - --title
...
- id: validate_register
  ...
  script: scripts/validate_register.py
  args:
  - '{{in}}'
  - --output
  - --strict
