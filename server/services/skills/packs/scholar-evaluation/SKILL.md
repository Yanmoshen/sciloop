...
stage: ideation
source: K-Dense-AI/scientific-agent-skills/skills/scholar-evaluation
steps:
- id: calculate_scores
...
- id: check_process
  title: check process
  script: scripts/check_process.py
  args:
  - '{{in}}'
  - --process
  - --output
...
- id: check_traceability
  ...
  script: scripts/check_traceability.py
  args:
  - '{{in}}'
  - --rubric
  - --evaluation
  ...
  - '{{out}}/output'
  required: false
- id: generate_report_scaffold
  title: generate report scaffold
  script: scripts/generate_report_scaffold.py
  args:
  - '{{in}}'
  - --rubric
  - --evaluation
  ...
  - '{{out}}/output'
  required: false
- id: summarize_agreement
  title: summarize agreement
  script: scripts/summarize_agreement.py
  args:
  - '{{in}}'
  - --rubric
  - --ratings
...
- id: validate_rubric
  ...
  script: scripts/validate_rubric.py
  args:
  - '{{in}}'
  - --rubric
  - --output
...
- id: weight_sensitivity
  ...
  script: scripts/weight_sensitivity.py
  args:
  - '{{in}}'
  - --rubric
  - --evaluation
