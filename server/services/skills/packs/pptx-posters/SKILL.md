...
stage: writing
source: K-Dense-AI/scientific-agent-skills/skills/pptx-posters
steps:
- id: check_layout
  title: check layout
...
- id: check_palette
  title: check palette
  script: scripts/check_palette.py
  args:
  - '{{in}}'
  - --output
  outputs:
...
- id: generate_poster
  ...
  script: scripts/generate_poster.py
  args:
  - '{{in}}'
  - --output
  - --report
...
- id: inspect_pptx
  title: inspect pptx
  script: scripts/inspect_pptx.py
  args:
  - '{{in}}'
  - --output
  outputs:
...
- id: inventory_images
  ...
  script: scripts/inventory_images.py
  args:
  - '{{in}}'
  - --output
  outputs:
