...
stage: figure
source: K-Dense-AI/scientific-agent-skills/skills/scientific-visualization
steps:
- id: export_plan
  title: export plan
  script: scripts/export_plan.py
  args:
  - '{{in}}'
  - --list
  - --publisher
...
- id: figure_export
  title: figure export
  script: scripts/figure_export.py
  args:
  - '{{in}}'
  - --demo
  - --formats
...
- id: image_metadata
  ...
  script: scripts/image_metadata.py
  args:
  - '{{in}}'
  - --format
  - --mode
...
- id: palette_audit
  title: palette audit
  script: scripts/palette_audit.py
  args:
  - '{{in}}'
  - --palette
  - --color
...
- id: style_presets
  title: style presets
  script: scripts/style_presets.py
  args:
  - '{{in}}'
  - --list
  - --show
...
- id: style_preview
  title: style preview
  script: scripts/style_preview.py
  args:
  - '{{in}}'
  - --output
  - --style
