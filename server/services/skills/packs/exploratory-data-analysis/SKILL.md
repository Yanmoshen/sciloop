...
stage: experiment
source: K-Dense-AI/scientific-agent-skills/skills/exploratory-data-analysis
steps:
- id: capability_manifest
  title: capability manifest
  script: scripts/capability_manifest.py
  args:
  - '{{in}}'
  - --output
  - --root
  ...
  - '{{out}}/output'
  required: false
- id: distribution_sensitivity
  title: distribution sensitivity
  script: scripts/distribution_sensitivity.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
...
- id: eda_analyzer
  title: eda analyzer
  script: scripts/eda_analyzer.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
...
- id: image_inspector
  ...
  script: scripts/image_inspector.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
  ...
  - '{{out}}/output'
  required: false
- id: missingness_leakage_audit
  title: missingness leakage audit
  script: scripts/missingness_leakage_audit.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
...
- id: report_scaffold
  ...
  script: scripts/report_scaffold.py
  args:
  - '{{in}}'
  - --input
  - --root
  ...
  - '{{out}}/output'
  required: false
- id: sequence_inspector
  title: sequence inspector
  script: scripts/sequence_inspector.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
  ...
  required: false
- id: tabular_profile
  title: tabular profile
  script: scripts/tabular_profile.py
  args:
  - '{{in}}'
  - --root
  - --max-bytes
