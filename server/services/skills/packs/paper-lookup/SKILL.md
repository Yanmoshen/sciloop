...
stage: literature
source: K-Dense-AI/scientific-agent-skills/skills/paper-lookup
steps:
- id: arxiv_atom
  title: arxiv atom
  script: scripts/arxiv_atom.py
  args:
  - '{{in}}'
  - -o
  - --ids-only
...
- id: jats_to_text
  title: jats to text
  script: scripts/jats_to_text.py
  args:
  - '{{in}}'
  - -o
  - --sections
  ...
  outputs: []
  required: false
- id: openalex_abstract
  title: openalex abstract
  script: scripts/openalex_abstract.py
  args:
  - '{{in}}'
  - -o
  - --text-only
