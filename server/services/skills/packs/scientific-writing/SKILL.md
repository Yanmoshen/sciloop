...
stage: writing
source: K-Dense-AI/scientific-agent-skills/skills/scientific-writing
steps:
- id: lint_manuscript
  title: lint manuscript
  script: scripts/lint_manuscript.py
  args:
  - '{{in}}'
  - --manifest
  outputs: []
  required: false
- id: scaffold_manuscript
  title: scaffold manuscript
  script: scripts/scaffold_manuscript.py
  args:
  - '{{in}}'
  - --output-dir
  - --document-id
