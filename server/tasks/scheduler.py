...
1. **默认关闭**：``SCHEDULER_ENABLED=0``（默认）时不创建调度器，只打一行明确日志
   ...
   （外部额度与成本护栏都要求这一点）。
2. **惰性 import，缺依赖只告警**：任务模块（``tasks.jobs.*``）与 APScheduler 都在
...
的 lifespan 调用（见 ``main.lifespan``）；本模块 import 时**不产生副作用**。
"""
...
#: 任务名同时是 APScheduler 的 ``job_id``，也是 ``tasks.jobs`` 下的模块名。
JOB_SPECS: tuple[tuple[str, str, str, str], ...] = (
    (
        "fetch_papers",
        "tasks.jobs.fetch_papers",
        "run_fetch_sync",
        "scheduler_fetch_interval_hours",
    ),
    (
        "score_papers",
        "tasks.jobs.score_papers",
        "run_score_job",
        "scheduler_score_interval_hours",
    ),
    (
        "parse_fulltext",
        "tasks.jobs.parse_fulltext",
