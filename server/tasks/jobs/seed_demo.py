...
::
    ...
    docker compose exec -T backend python -m tasks.jobs.seed_demo --dry-run
    docker compose exec -T backend python -m tasks.jobs.seed_demo --run-pipeline
    docker compose exec -T backend python -m tasks.jobs.seed_demo --verify
...
def _check_snapshot() -> SeedCheck:
    ...
    return SeedCheck(
        ...
        next_step=None
        if ready
        else "python -m services.demo.snapshot --replace",
    )
...
def _check_fixtures() -> SeedCheck:
    ...
    return SeedCheck(
        ...
        next_step=None
        if ready
        else "python -m services.demo.replay --record --project-id <id>（需 LLM 凭据）",
    )
...
def run_seed(
    *,
    ...
    report["resume_entrypoints"] = [
        "python -m tasks.jobs.seed_demo --dry-run           # 体检（只读）",
        "python -m tasks.jobs.seed_demo --run-pipeline       # 真实跑一轮并固化",
        "python -m services.demo.snapshot --replace          # 只固化论文库快照",
        "python -m services.demo.replay --record --project-id "
        f"{project_id}   # 录制 LLM fixture（需凭据）",
        "python -m services.demo.replay --replay-check --project-id "
...
def _build_snapshot() -> dict[str, Any]:
    try:
        from services.demo.snapshot import build_snapshots
